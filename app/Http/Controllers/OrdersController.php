...
class OrdersController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            $q_order = Order::select('*')
                            ->orderByDesc('order_id');
            return Datatables::of($q_order)
                    ->addIndexColumn()
                    ->addColumn('action', function($row){
     
                        //$btn = '<div data-toggle="tooltip"  data-id="'.$row->id.'" data-original-title="Edit" class="btn btn-sm btn-icon btn-outline-success mr-2 edit editOrder">Edit</div>';
                        $btn = '<div data-toggle="tooltip"  data-id="'.$row->order_id.'" data-original-title="Delete" class="btn btn-sm btn-icon btn-outline-danger mr-2 deleteOrder">Delete</div>';
        ...
        $orders = Order::select('*')->orderBy('order_id')->get();
        $data = [
            'orders' => $orders        ];
